...
export async function GET(request) {
  try {
    ...
    const { data, error } = await supabase
      ...
      .eq("practice_location_providers.providers_info.added_by", addedById); // Filter by added_by in the joined providers_info table

    if (error) throw error;

    // Filter out locations where practice_location_providers is an empty array
    const filteredData = data.filter(location => location.practice_location_providers.length > 0);
