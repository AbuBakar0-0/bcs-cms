    ...
    await Promise.all(
      [
        ...
        insertPortalCredential(
          "bankLogin",
          formData[`bankLoginUsername`],
          formData[`bankLoginPassword`],
          {
            platformName: formData.bankName,
          }
        ),
        insertPortalCredential(
          "caqh",
...
export async function GET(request) {
  try {
    ...
    console.log("PORATATATAA:",portalCredentials);
    if (portalError) {
      throw new Error(
...
function transformPortalCredentials(credentials) {
  ...
  credentials?.forEach((cred) => {
    ...
    if (cred.type === "caqh") {
      ...
      portalData.caqhExpiryDate = cred.expiry_date || "";
    ...
    } else if (cred.type === "bankLogin") {
      portalData.bankName = cred.platform_name || "";
      portalData.bankLoginUsername = cred.username || "";
      portalData.bankLoginPassword = cred.password || "";
    } else {
