...
import { InsertEducationTraining } from "./insertEducationTraining";
        ...
        reader.onload = async (event) => {
            ...
            try {
                ...
                for (let row of rows) {
                    const {
                        //providers_info
                        first_name, middle_initial, last_name, provider_title, ssn, gender, dob, birth_city, birth_state, birth_country, license_id, state_issued, issue_date, expiry_date,

                        //home_address
                        home_address_line_1, home_address_line_2, home_address_city, home_address_state, home_address_zip_code,

                        //service_location_address
                        service_location_address_line_1, service_location_address_line_2, service_location_address_city, service_location_address_state, service_location_address_zip_code,

                        //mailing_address
                        mailing_address_line_1, mailing_address_line_2, mailing_address_city, mailing_address_state, mailing_address_zip_code,

                        //contact info
                        personal_home_phone, personal_cell_phone, personal_email, personal_work_email,

                        //emergency_contact_info
                        emergency_contact_name, emergency_contact_relation, emergency_contact_cell_phone, emergency_contact_email,
                    } = row;


                    if (last_name != undefined) { // STATES END OF FILE (CSV)

                        var providerId = null;
                        console.log("HELLO : DATA IS ====> ",first_name, middle_initial, last_name, provider_title);
                        console.log(first_name != "" && middle_initial != "" && last_name != "" && provider_title != "");
                        if (first_name != "" && middle_initial != "" && last_name != "" && provider_title != "") {  // CHECK FOR SAME PROVIDER
                            ...
                            const { data: providerData, error: providerError } = await supabase.from("providers_info").insert({
                                added_by: userUuid,
                            ...
                            //==========================================PROFESSIONAL IDS ALL DATA INSERTION==========================================//

                            await InsertProfessionalIds({ providerData: providerData, row: row });
                        ...
                        //==========================================EDUCATION & TRAINING ALL DATA INSERTION==========================================//
                        // await InsertEducationTraining({ providerData: providerData, row: row });
